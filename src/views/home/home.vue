...
import img from '../../assets/images/noSource.png';
import { imgList } from '../../libs/util.js';
import PullTo from 'vue-pull-to';
import { setJsBridge } from '../../config/jsBridge.js';
export default {
  name: 'home',
  data() {
    return {
  ...
  created() {
    let that = this;
    this.balanceType = this.$route.params.type;
  ...
  methods: {
    ...
    getOrderList() {
      ...
      if (this.selectorValue !== '全部') {
        params.accessTag = this.selectorValue == '支出' ? 1 : 0;
      }
      if (this.payFeeModeCode !== 0) {
        params.chargeType = this.payFeeModeCode;
      }
      if (this.$route.query.appid) {
        params.useAppId = this.$route.query.appid;
      }
